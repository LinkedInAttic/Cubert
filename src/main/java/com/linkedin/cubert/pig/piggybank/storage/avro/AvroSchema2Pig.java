...
public class AvroSchema2Pig
{
     ...
     */
    private static ResourceFieldSchema inconvert(Schema in,
                                                 String fieldName,
                                                 Set<Schema> visitedRecords) throws IOException
    {
        ...
        if (avroType.equals(Schema.Type.RECORD))
        {
            ...
            else
            {
                ...
                ResourceFieldSchema[] childFields =
                        new ResourceFieldSchema[fields.size()];
                int index = 0;
                for (Schema.Field field : fields)
                {
                    childFields[index++] =
                            inconvert(field.schema(), field.name(), visitedRecords);
                }
        ...
        else if (avroType.equals(Schema.Type.ARRAY))
        {
            ...
            ResourceFieldSchema subFieldSchema =
                    inconvert(elemSchema, ARRAY_FIELD, visitedRecords);
        ...
        else if (avroType.equals(Schema.Type.MAP))
        {
            ...
            ResourceFieldSchema valueSchema =
                    inconvert(in.getValueType(), in.getName(), visitedRecords);

            if (valueSchema.getSchema() != null)
            {
                fieldSchema.setSchema(valueSchema.getSchema());
            }
            else
            {
                FieldSchema fs = new FieldSchema(in.getName(), valueSchema.getType());
                org.apache.pig.impl.logicalLayer.schema.Schema ss =
                        new org.apache.pig.impl.logicalLayer.schema.Schema(fs);

                ResourceSchema elemSchema = new ResourceSchema(ss);
                fieldSchema.setSchema(elemSchema);
            }
        }
        else if (avroType.equals(Schema.Type.UNION))
        {
            ...
            if (AvroStorageUtils.isAcceptableUnion(in))
            {
                ...
                ResourceFieldSchema realFieldSchema =
                        inconvert(acceptSchema, null, visitedRecords);
